...
pub mod default {
    ...
    pub static DEFAULT_POLICIES: LazyLock<[(&'static str, Policy); 10]> = LazyLock::new(|| {
        [
            (
                "user-isolated",
                Policy {
...
"#;
    ...
    #[tokio::test]
    async fn test_user_isolated_canned_policies() {
        let conditions = HashMap::new();
        let claims = HashMap::new();
        let user = "alice";

        let isolated_policy = default_policy("user-isolated");
        let isolated_readonly_policy = default_policy("user-isolated-readonly");

        // Alice accessing her own bucket `alice-data`
        let alice_own_bucket_args = Args {
            account: user,
            groups: &None,
        ...
        assert!(isolated_policy.is_allowed(&alice_own_bucket_args).await);
        assert!(isolated_readonly_policy.is_allowed(&alice_own_bucket_args).await);

        // Alice putting an object in `alice-data`
        let alice_put_object_args = Args {
            account: user,
            groups: &None,
            ...
            conditions: &conditions,
            is_owner: false,
            object: "photos/vacation.png",
            claims: &claims,
            deny_only: false,
        };
        assert!(isolated_policy.is_allowed(&alice_put_object_args).await);
        assert!(!isolated_readonly_policy.is_allowed(&alice_put_object_args).await); // Readonly should deny PutObject

        // Alice attempting to access Bob's bucket `bob-data` -> must be denied!
        let alice_bob_bucket_args = Args {
            account: user,
            groups: &None,
            action: Action::S3Action(S3Action::ListBucketAction),
            bucket: "bob-data",
        ...
        assert!(!isolated_policy.is_allowed(&alice_bob_bucket_args).await);
        assert!(!isolated_readonly_policy.is_allowed(&alice_bob_bucket_args).await);
    }
